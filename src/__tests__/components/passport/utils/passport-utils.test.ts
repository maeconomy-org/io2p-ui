...
describe('passport-utils', () => {
  describe('flattenValues', () => {
    it('joins multiple non-empty values with a comma', () => {
      expect(
        flattenValues({
          values: [{ data: 'Steel' }, { data: 'Polymer' }, { data: '' }],
        })
      ).toBe('Steel, Polymer')
    })
    ...
    it('coerces non-string values', () => {
      expect(flattenValues({ values: [{ data: 42 }, { data: 3.14 }] })).toBe(
        '42, 3.14'
      )
    })
  })
  ...
  describe('findValueByKey', () => {
    const props = [
      { key: 'manufacturer', values: [{ data: 'Acme' }] },
      { key: 'weight', values: [{ data: '1240 kg' }] },
    ]
  ...
  describe('groupPropertiesByCategory', () => {
    it('buckets known dictionary keys into the right category and skips empties', () => {
      const groups = groupPropertiesByCategory([
        { key: 'manufacturer', values: [{ data: 'Acme' }] },
        { key: 'weight', values: [{ data: '10' }] },
        { key: 'height', values: [{ data: '' }] }, // empty -> dropped
        { key: 'co2-equivalent', values: [{ data: '500' }] },
      ])
    ...
    it('excludes lifecycle keys (they are surfaced by the ribbon, not the cards)', () => {
      const groups = groupPropertiesByCategory([
        { key: 'production-date', values: [{ data: '2020-01-01' }] },
        { key: 'warranty-end', values: [{ data: '2030-01-01' }] },
        { key: 'manufacturer', values: [{ data: 'Acme' }] },
      ])
    ...
    it('routes free-text properties into the "other" bucket', () => {
      const groups = groupPropertiesByCategory([
        { key: 'custom-thing', values: [{ data: '123' }] },
      ])
