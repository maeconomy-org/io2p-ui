...
describe('createLogger sink routing', () => {
  ...
  it('security() and import() stamp category and prefix', () => {
    const c = collectorSink()
    const logger = createLogger([{ sink: c.sink, threshold: () => 'debug' }])
    logger.security('rate_limit_exceeded', { max: 5 })
    logger.import('job_started', { jobId: 'j1' }, 'debug')

    expect(c.records[0].msg).toBe('[SECURITY] rate_limit_exceeded')
    expect(c.records[0].level).toBe('warn')
    expect(c.records[0].category).toBe('security')
    expect(c.records[0].max).toBe(5)
    expect(c.records[1].msg).toBe('[IMPORT] job_started')
    expect(c.records[1].level).toBe('debug')
    expect(c.records[1].category).toBe('import')
  })
})
