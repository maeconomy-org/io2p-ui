            ...
            {problems.slice(0, PROBLEM_LIMIT).map((problem, index) => {
              ...
              return (
                <li key={index}>
                  ...
                  {/* The node names the field when it knows which one, and two of the three names
                      it uses are not columns anyone mapped: `body` means the item as a whole, and
                      `tempId` is generated by the import itself — whose VALUE is already printed
                      two spans back, so naming it reads as "(A-B) tempId:", the value then the
                      word for it. `parents` is the only one a reader can act on. */}
                  {problem.field &&
                    problem.field !== 'body' &&
                    problem.field !== 'tempId' && (
