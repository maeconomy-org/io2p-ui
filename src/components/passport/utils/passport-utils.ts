...
/**
 * A property as io2p returns it — `id`/`data`, not the retired node's `uuid`/`value`.
 *
 * The passport used to receive a translated shape from a mapper hook. Speaking the API's own
 * vocabulary removes the mapper, and with it the class of bug where a renderer reads a field the
 * translation forgot to fill.
 */
export interface NormalizedProperty {
  id?: string
  key?: string
  label?: string
  values?: Array<{ id?: string; data?: unknown }>
}
...
export function flattenValues(property: NormalizedProperty): string {
  const parts = (property.values ?? [])
    .map((v) =>
      v?.data === undefined || v?.data === null ? '' : String(v.data)
    )
    .map((s) => s.trim())
    .filter(Boolean)
  return parts.join(', ')
