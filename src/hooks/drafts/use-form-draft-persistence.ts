// PARKED FEATURE. Form drafts have no live consumer — `auth-context` only clears them on logout.
// Kept because drafts are coming back (user, 2026-07-30); this file needs a pass for the React
// Compiler rules and a rewire to io2p when they are.
//
'use client'
...
export function useFormDraftPersistence<T extends Record<string, any>>({
  ...
  // PARKED — see the note at the top of this file. The compiler lint objects to the render-time ref
  // writes below, and it is right in general; the disables are here because the alternative it wants
  // (a `useEffect`) is the exact bug this code was written to fix, and drafts have no live consumer
  // to verify a rewrite against. Fix this when drafts are wired back up, with tests.
  //
  ...
  /* eslint-enable react-hooks/refs, react-hooks/set-state-in-effect */
