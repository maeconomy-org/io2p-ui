...
// keeps `@/lib/logger` safe inside the client module graph: 46 of the 58
// importers are 'use client' components.

import { redactValue, redactPresignedUrlString } from '@/lib/redact'
...
export function createLogger(
  ...
  function categoryLog(
    ...
    emit(level, `[${category.toUpperCase()}] ${event}`, {
      category,
      event,
      ...details,
    })
  ...
  return {
    ...
    security: (event, details, level = 'warn') =>
      categoryLog('security', event, details, level),
    import: (event, details, level = 'info') =>
      categoryLog('import', event, details, level),
