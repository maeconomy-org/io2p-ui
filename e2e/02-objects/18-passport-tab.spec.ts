...
test.describe('18 - Product Passport Sheet', () => {
  ...
  test('renders a rich passport for a window frame with active warranty', async ({
    ...
    // Footer offers Download PDF and Close. The PDF is rendered server-side by
    // /api/passport/[uuid]/pdf, so there is no second tab and no print dialog.
    await expect(
      passport.locator('[data-testid="passport-download-pdf-button"]')
    ).toBeVisible()
    await expect(
